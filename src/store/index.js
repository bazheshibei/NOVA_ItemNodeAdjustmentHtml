  ...
  actions: {
    ...
    A_nextBatchAdjusmentItemGantt({ state, commit }) {
      ...
      if (page_type === 'add') {
        ...
        //
        const { accessDataList = [], adjusmentAuditMapList = [], adjustment_reason = '', adjustment_remark = '', adjustmentReason = [], business_type = '', ganttTemplateList = [], itemSummaryItemData = {}, itemSummaryDataList = [], nodeData = [], node_business_type_id = '', gantt_type = '3', item_name, item_id, adjustment_id } = res
        const { ganttTemplateNewList, templateObj } = Tool.ganttTemplateListAddAttr(ganttTemplateList)
        state.adjustmentReason = adjustmentReason //            变更原因 -
        state.business_type = business_type //                  业务类型 -
        state.ganttTemplateList = ganttTemplateNewList //       甘特表模板 -
    ...
    A_saveItemNodeAdjuestment({ state, getters }, { audit_result }) {
      ...
      const { adjustment_id, item_id, node_business_type_id, gantt_type, adjustment_reason, adjustment_remark, file } = state
